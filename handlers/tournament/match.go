...
func (h *Handler) renderMatchesSection(w http.ResponseWriter, r *http.Request, logger *slog.Logger, idTournament int, userId int, withHXTrigger bool) {
	...
	availableTeams, err := tournamentModel.GetAvailableTeamsForTournament(r.Context(), idTournament, userId)
	if err != nil {
		logger.Error(err.Error())
		httpError.InternalError(w)
		return
...
func (h *Handler) resolveMatchWinner(w http.ResponseWriter, r *http.Request, logger *slog.Logger, idMatch int, idTournament int, scoreTeam1 int, scoreTeam2 int, userId int) {
	...
	if winnerId != nil {
		...
		if err == nil && allRoundFinished {
			nextAvailable, err := tournamentModel.GetAvailableTeamsForTournament(r.Context(), idTournament, userId)
...
func parseTeamDelta(r *http.Request) (team int, delta int, err error) {
	team, err = strconv.Atoi(r.FormValue("team"))
	if err != nil || (team != 1 && team != 2) {
		return 0, 0, fmt.Errorf("invalid team")
	}
	delta, err = strconv.Atoi(r.FormValue("delta"))
	if err != nil || (delta != 1 && delta != -1) {
		return 0, 0, fmt.Errorf("invalid delta")
	}
	return team, delta, nil
}
...
func (h *Handler) HandleCreateMatch(w http.ResponseWriter, r *http.Request) {
	...
	h.renderMatchesSection(w, r, logger, idTournament, user.Id, true)
}
...
func (h *Handler) HandleUpdateMatchScore(w http.ResponseWriter, r *http.Request) {
	...
	h.resolveMatchWinner(w, r, logger, idMatch, idTournament, scoreTeam1, scoreTeam2, user.Id)
	h.renderMatchesSection(w, r, logger, idTournament, user.Id, true)
}
...
func (h *Handler) HandleUpdateRoundBo(w http.ResponseWriter, r *http.Request) {
	user, _ := auth.UserFromRequest(r)
	logger := h.Slog.With("userId", user.Id)
	vars := mux.Vars(r)
	idTournament, _ := strconv.Atoi(vars["idTournament"])
	round, _ := strconv.Atoi(vars["round"])
	...
	if err := tournamentModel.UpdateMatchsBo(r.Context(), idTournament, round, boFormat); err != nil {
		logger.Error(err.Error())
		httpError.InternalError(w)
		return
	}

	logger.Info("round BO updated", "tournamentId", idTournament, "round", round, "boFormat", boFormat)
...
func (h *Handler) HandleUpdateMatchKills(w http.ResponseWriter, r *http.Request) {
	...
	team, delta, err := parseTeamDelta(r)
	if err != nil {
	...
	h.renderMatchesSection(w, r, logger, idTournament, user.Id, false)
}
...
func (h *Handler) HandleIncrementMatchScore(w http.ResponseWriter, r *http.Request) {
	...
	h.resolveMatchWinner(w, r, logger, idMatch, idTournament, m.ScoreTeam1, m.ScoreTeam2, user.Id)
	h.renderMatchesSection(w, r, logger, idTournament, user.Id, true)
}
